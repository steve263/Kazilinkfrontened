...
import { X, Upload, CheckCircle, Star } from "lucide-react";
...
export default function VideoUploadModal({ isOpen, onClose, userRole, onPosted }: VideoUploadModalProps) {
  ...
  // Fetch booked providers for customers
  useEffect(() => {
            ...
            <div>
              {!file ? (
                ...
                <div className="space-y-4">
                  {previewUrl && (
                    <video src={previewUrl} className="w-full rounded-xl max-h-48 object-contain bg-black" muted />
                  )}
                  <div>
                    ...
                    <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-kazi-orange rounded-full transition-all duration-200"
                        style={{ width: `${uploadProgress}%` }}
                      />
                    </div>
                    <p className="text-gray-400 text-xs mt-2 text-center">Please wait while your video uploads to the cloud…</p>
                  </div>
                </div>
              ) : null}
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              {previewUrl && (
                <video
                  src={previewUrl}
                  className="w-full rounded-xl max-h-40 object-contain bg-black"
                  muted
                  controls
                />
              )}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-2 bg-kazi-orange text-white text-xs font-black rounded-xl disabled:opacity-50 hover:bg-orange-600"
                  >
                    Add
                  </button>
                </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {hashtags.map((h) => (
                      <span key={h} className="flex items-center gap-1 px-2.5 py-1 bg-orange-500/20 text-kazi-orange text-xs font-bold rounded-full border border-orange-500/30">
                        #{h}
                        <button onClick={() => setHashtags(hashtags.filter((x) => x !== h))}>
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))}
                      ...
                      {[1, 2, 3, 4, 5].map((n) => (
                        ...
                        >
                          <Star
                            ...
                            color={(hoverRating || rating) >= n ? "#F59E0B" : "#6B7280"}
                          />
                        </button>
                      ))}
                    </div>
                  ...
                  {bookedProviders.length > 0 && (
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-kazi-orange"
                      >
                        <option value="" className="bg-gray-900">Select a provider (optional)</option>
                        {bookedProviders.map((p) => (
                          <option key={p.id} value={p.id} className="bg-gray-900">
                            {p.businessName} · {p.category}
                          </option>
                        ))}
                      </select>
                    </div>
